...
export class OrderMenuDetailsComponent implements OnInit, CartEventListener {
  ...
  constructor(private cartService: CartService, public constants: Constants) {
  ...
  changeMethod(method) {
    if (this.method !== method) {
      ...
      $('#order-method-slider').prop(
        'checked',
        !$('#order-method-slider').prop('checked')
      );
    }
  }
  ...
  canOrder() {
    ...
    if (this.cart.length === 0) {
      ...
      this.method === this.constants.orderMethods.order &&
      parseFloat(this.getCartPrice()) < 50
    ) {
